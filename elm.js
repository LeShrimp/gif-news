...
Elm.Giphy = Elm.Giphy || {};
Elm.Giphy.make = function (_elm) {
   "use strict";
   _elm.Giphy = _elm.Giphy || {};
   if (_elm.Giphy.values) return _elm.Giphy.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var decodeTranslateUrl = A2($Json$Decode.at,_U.list(["data","images","fixed_height","url"]),$Json$Decode.string);
   var decodeRandomUrl = A2($Json$Decode.at,_U.list(["data","image_url"]),$Json$Decode.string);
   var errorToErrorMessage = function (error) {
      return $Task.fail(function () {
         var _p0 = error;
         switch (_p0.ctor)
         {case "Timeout": return "Connection timed out";
            case "NetworkError": return "Network error";
            case "UnexpectedPayload": return A2($Basics._op["++"],"UnexpectedPayload: ",_p0._0);
            default: return A2($Basics._op["++"],"Bad response: ",A2($Basics._op["++"],$Basics.toString(_p0._0),A2($Basics._op["++"],", ",_p0._1)));}
      }());
   };
   ...
   var getRandomGif = function (topic) {    return A2($Task.onError,A2($Http.get,decodeRandomUrl,randomUrl(topic)),errorToErrorMessage);};
   var translateUrl = function (phrase) {
      return A2($Http.url,"http://api.giphy.com/v1/gifs/translate",_U.list([A2(_op["=>"],"api_key","dc6zaTOxFJmzC"),A2(_op["=>"],"s",phrase)]));
   };
   var getTranslateGif = function (phrase) {    return A2($Task.onError,A2($Http.get,decodeTranslateUrl,translateUrl(phrase)),errorToErrorMessage);};
   return _elm.Giphy.values = {_op: _op,getRandomGif: getRandomGif,getTranslateGif: getTranslateGif};
};
Elm.MiniScraper = Elm.MiniScraper || {};
Elm.MiniScraper.make = function (_elm) {
   "use strict";
   _elm.MiniScraper = _elm.MiniScraper || {};
   if (_elm.MiniScraper.values) return _elm.MiniScraper.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Http = Elm.Http.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var rawErrorToErrorMessage = function (error) {
      var _p0 = error;
      if (_p0.ctor === "RawTimeout") {
            return $Task.fail("Connection timed out");
         } else {
            return $Task.fail("Network error");
         }
   };
   var decodeResponse = function (response) {
      var errorDecoder = A2($Json$Decode._op[":="],"error",$Json$Decode.string);
      var tryDecodeError = function (str) {
         var _p1 = A2($Json$Decode.decodeString,errorDecoder,str);
         if (_p1.ctor === "Ok") {
               return $Task.fail(_p1._0);
            } else {
               return $Task.fail(A2($Basics._op["++"],"Unable to decode response: ",str));
            }
      };
      var successDecoder = A2($Json$Decode._op[":="],"result",$Json$Decode.list($Json$Decode.string));
      var decodeResponseText = function (str) {
         var _p2 = A2($Json$Decode.decodeString,successDecoder,str);
         if (_p2.ctor === "Ok") {
               return $Task.succeed(_p2._0);
            } else {
               return tryDecodeError(str);
            }
      };
      if (_U.cmp(200,response.status) < 1 && _U.cmp(response.status,300) < 0) {
            var _p3 = response.value;
            if (_p3.ctor === "Text") {
                  return decodeResponseText(_p3._0);
               } else {
                  return $Task.fail("Response body is a blob, expecting a string.");
               }
         } else return $Task.fail(A2($Basics._op["++"],$Basics.toString(response.status),A2($Basics._op["++"]," ",response.statusText)));
   };
   _op["=>"] = F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};});
   var requestBody = F2(function (url,selector) {
      return A2($Json$Encode.encode,
      0,
      $Json$Encode.object(_U.list([A2(_op["=>"],"url",$Json$Encode.string(url)),A2(_op["=>"],"selector",$Json$Encode.string(selector))])));
   });
   var postRequest = F2(function (url,selector) {
      return A2($Http.send,
      $Http.defaultSettings,
      {verb: "POST"
      ,headers: _U.list([A2(_op["=>"],"Content-Type","application/json")])
      ,url: "http://www.thereisbeauty.de/mini-scraper/"
      ,body: $Http.string(A2(requestBody,url,selector))});
   });
   var scrapeTexts = F2(function (url,selector) {
      return A2($Task.andThen,A2($Task.onError,A2(postRequest,url,selector),rawErrorToErrorMessage),decodeResponse);
   });
   return _elm.MiniScraper.values = {_op: _op,scrapeTexts: scrapeTexts};
};
Elm.Native.Effects = {};
...
Elm.MainLogic = Elm.MainLogic || {};
Elm.MainLogic.make = function (_elm) {
   "use strict";
   _elm.MainLogic = _elm.MainLogic || {};
   if (_elm.MainLogic.values) return _elm.MainLogic.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   $Giphy = Elm.Giphy.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Html$Attributes = Elm.Html.Attributes.make(_elm),
   $Html$Events = Elm.Html.Events.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $MiniScraper = Elm.MiniScraper.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $String = Elm.String.make(_elm),
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var currentHeadline = function (model) {    return A2($Maybe.withDefault,"",$List.head(A2($List.drop,model.index,model.headlines)));};
   var groups = F2(function (n,xs) {
      var _p0 = xs;
      if (_p0.ctor === "[]") {
            return _U.list([]);
         } else {
            var _p1 = _p0;
            return A2($List._op["::"],A2($List.take,n,_p1),A2(groups,n,A2($List.drop,n,_p1)));
         }
   });
   var headlineToPhrases = function (headline) {
      return A2($List.map,$String.join(" "),A2(groups,2,A2($List.filter,function (s) {    return _U.cmp($String.length(s),3) > 0;},$String.words(headline))));
   };
   var Next = {ctor: "Next"};
   var view = F2(function (address,model) {
      var renderImages = A2($List.map,function (url) {    return A2($Html.img,_U.list([$Html$Attributes.src(url)]),_U.list([]));},model.gifUrls);
      return A2($Html.div,
      _U.list([A2($Html$Events.onClick,address,Next)]),
      _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text(currentHeadline(model))])),A2($Html.div,_U.list([]),renderImages)]));
   });
   var NewGifUrls = function (a) {    return {ctor: "NewGifUrls",_0: a};};
   var fetchGifUrls = function (headline) {
      return $Effects.task(A2($Task.map,NewGifUrls,$Task.toMaybe($Task.sequence(A2($List.map,$Giphy.getTranslateGif,headlineToPhrases(headline))))));
   };
   var update = F2(function (action,model) {
      var _p2 = action;
      switch (_p2.ctor)
      {case "NewHeadlines": var newModel = _U.update(model,{headlines: A2($Maybe.withDefault,model.headlines,_p2._0),index: 0});
           return {ctor: "_Tuple2",_0: newModel,_1: fetchGifUrls(currentHeadline(newModel))};
         case "NewGifUrls": return {ctor: "_Tuple2",_0: _U.update(model,{gifUrls: A2($Maybe.withDefault,model.gifUrls,_p2._0)}),_1: $Effects.none};
         default: var newModel = _U.update(model,{index: A2($Basics._op["%"],model.index + 1,$List.length(model.headlines)),gifUrls: _U.list([])});
           return {ctor: "_Tuple2",_0: newModel,_1: fetchGifUrls(currentHeadline(newModel))};}
   });
   var NewHeadlines = function (a) {    return {ctor: "NewHeadlines",_0: a};};
   var fetchHeadlines = $Effects.task(A2($Task.map,
   NewHeadlines,
   $Task.toMaybe(A2($MiniScraper.scrapeTexts,"http://edition.cnn.com/",".cn--idx-1 .cd__headline .cd__headline-text"))));
   var Model = F3(function (a,b,c) {    return {headlines: a,index: b,gifUrls: c};});
   var init = {ctor: "_Tuple2",_0: A3(Model,_U.list([]),0,_U.list([])),_1: fetchHeadlines};
   return _elm.MainLogic.values = {_op: _op,init: init,view: view,update: update};
};
...
Elm.Main.make = function (_elm) {
   ...
   $Effects = Elm.Effects.make(_elm),
   $List = Elm.List.make(_elm),
   $MainLogic = Elm.MainLogic.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var app = $StartApp.start({init: $MainLogic.init,view: $MainLogic.view,update: $MainLogic.update,inputs: _U.list([])});
   var main = app.html;
